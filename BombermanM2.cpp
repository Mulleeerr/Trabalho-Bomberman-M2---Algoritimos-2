...
///===================================FIM FUNCAO LER MAPA==================================
    ...
    void gerarPoder(Poder& poder, int **m,Mapas mapa, clock_t& fim, Bomba& bomba) { 
        if (!poder.ativo && fim / CLOCKS_PER_SEC % 20 == 0 && !(bomba.raio > 1 || bomba.atravessaParedes)) { 
            do {
...
///=================================== FIM FUNCOES PODER==================================
    void salvarJogo(Jogador& player, Bomba& bomba, Inimigos inimigo[], int &numInimigos, Mapas &mapa, int** m) {
        ofstream arquivo("jogo_salvo.txt");
        if (arquivo.is_open()) {
            // Salvar informações do jogador
            arquivo << player.x << " " << player.y << endl;

            // Salvar informações da bomba
            arquivo << bomba.x << " " << bomba.y << " " << bomba.ativa << " " << bomba.tempoExplosao << endl;

            // Salvar informações dos inimigos
            arquivo << numInimigos << endl;
            for (int i = 0; i < numInimigos; i++) {
                arquivo << inimigo[i].x << " " << inimigo[i].y << " " << inimigo[i].vivo << endl;
            }

            // Salvar informações do mapa
            arquivo << mapa.linha << " " << mapa.coluna << endl;
            for (int i = 0; i < mapa.linha; i++) {
                for (int j = 0; j < mapa.coluna; j++) {
                    arquivo << m[i][j] << " ";
                }
            ...
            cout << "Nao foi possivel abrir o arquivo para salvar o jogo." << endl;
        }
}

    void carregarJogo(Jogador &player, Bomba &bomba, Inimigos inimigo[], int &numInimigos, Mapas &mapa, int** &m) {
        ...
        if (arquivo.is_open()) {
            ...
            cout << "Nao foi possivel abrir o arquivo de jogo salvo." << endl;
        }
}

    void mapa_salvo(Mapas mapa,int **m) {
        ofstream mapa_salvo("mapa_salvo.txt");
        if (mapa_salvo.is_open()) {
            // Salvar informações do mapa
            mapa_salvo << mapa.linha << " " << mapa.coluna << endl;
            for (int i = 0; i < mapa.linha; i++) {
                for (int j = 0; j < mapa.coluna; j++) {
                    mapa_salvo << m[i][j] << " ";
                }
    ...
    void menuJogo(Mapas &mapa, Jogador &player, Bomba &bomba, Inimigos inimigo[], int numInimigos, clock_t &fim, char &tecla, int** &m) {
        int escolha1;
        ...
        switch (escolha1) {
            ...
            case 2:
                if (bomba.ativa) {
                    cout << "Nao e possível salvar o jogo pois ha uma bomba no mapa." << endl;
                    system("pause");
                } else {
                    salvarJogo(player, bomba, inimigo, numInimigos, mapa, m);
                    system("pause");
                }
                break;
            case 3:
    ...
    void jogo(int **m, int &numInimigos, Inimigos inimigo[],char &tecla,Jogador &player,
        Bomba &bomba,Poder &poder,clock_t &inicio, clock_t &fim, int InimigosRestantes,clock_t &tempo_antes_pausa,clock_t &tempo_corrido, int save){
    ...
    for (int i = 0; i < numInimigos; i++){
        ...
        string c = "mapa_salvo.txt";
        const char* mapa_salvo = c.c_str();
        m = lerMapa(mapa_salvo,mapa);
    }
        system("cls");
    ...
    SetConsoleCursorPosition(GetStdHandle(STD_OUTPUT_HANDLE), coord); //COLOCAR CURSOR NO INICIO DA TELA
        ...
        if(mapa.JogoPausado == true) {
            system("cls");
            menuJogo(mapa,player,bomba,inimigo,numInimigos,fim,tecla,m);
    ...
    cout <<"Raio de Explosao Bomba: " << bomba.raio << endl;
    ...
    while (opcao != 5) {
        ...
        switch (opcao) {
            case 1:
                ...
                system("cls");
                save = 1;
                jogo(m,numInimigos,inimigo,tecla,player,bomba,poder,inicio,fim,InimigosRestantes,tempo_antes_pausa,tempo_corrido,save);
                break;
            case 2:
                carregarJogo(player, bomba,inimigo,numInimigos,mapa,m);
                save = 0;
                cout << "Jogo carregado com sucesso!" << endl;
                mapa_salvo(mapa,m);
                system("pause");
                jogo(m,numInimigos,inimigo,tecla,player,bomba,poder,inicio,fim,InimigosRestantes,tempo_antes_pausa,tempo_antes_pausa,save);
                break;
            case 3:
                salvarJogo(player, bomba, inimigo, numInimigos,mapa,m);
                break;
